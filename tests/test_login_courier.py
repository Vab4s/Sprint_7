...
class TestLoginCourier:
    ...
    @allure.step('Проверка авторизации курьера корректными данными')
    ...
    @allure.step('Проверка авторизации курьера c указанием некорректного логина')
    ...
    @allure.step('Проверка авторизации курьера c указанием некорректного пароля')
    def test_incorrect_password(self):
        payload = {'login': self.login, 'password': generate_random_string()}
        response = requests.post(LOGIN, data=payload)
        assert response.status_code == 404 and response.json()['message'] == ERROR_ACCOUNT_NOT_FOUND

    @allure.step('Проверка авторизации курьера c без логина')
    ...
    @allure.step('Проверка авторизации курьера c без пароля')
    def test_login_without_password(self):
        payload = {'login': self.login, 'password': ''}
        response = requests.post(LOGIN, data=payload)
        assert response.status_code == 400 and response.json()['message'] == ERROR_INSUFFICIENT_LOGIN_DATA

    @allure.step('Проверка авторизации несуществующего курьера')
